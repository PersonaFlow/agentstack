...
def get_thread_repository(session: AsyncSession = Depends(get_postgresql_session_provider)):
...
class ThreadRepository(BaseRepository):
    ...
    async def create_thread(self, data: dict) -> Thread:
        ...
        try:
            ...
            await logger.exception("Failed to create thread due to a database error", exc_info=True, thread_data=data)
            raise HTTPException(status_code=400, detail="Failed to create thread.") from e

    @staticmethod
    def _get_retrieve_query() -> select:
        """A private method to construct the default query for thread retrieval."""
        return select(Thread.id,
                      Thread.user_id,
                      Thread.assistant_id,
                      Thread.name,
    ...
    async def retrieve_threads(self, filters: Optional[dict[str, Any]] = None) -> list[Thread]:
        """Fetches all threads."""
        try:
            ...
            await logger.exception("Failed to retrieve threads due to a database error", exc_info=True)
    ...
    async def retrieve_thread(self, thread_id) -> Thread:
        ...
        try:
            ...
            await logger.exception("Failed to retrieve thread due to a database error", exc_info=True, thread_id=thread_id)
    ...
    async def retrieve_by_thread_id(self, thread_id) -> Thread:
        ...
        try:
            ...
            await logger.exception("Failed to retrieve thread by thread_id due to a database error", exc_info=True, thread_id=thread_id)
            raise HTTPException(status_code=500, detail="Failed to retrieve thread by thread_id.")
    ...
    async def update_thread(self, thread_id: str, data: dict) -> Thread:
        ...
        try:
            ...
            await self.postgresql_session.rollback()
            await logger.exception("Failed to update thread due to a database error", exc_info=True, thread_id=thread_id, thread_data=data)
    ...
    async def delete_thread(self, thread_id: str) -> Thread:
        ...
        try:
            ...
            await self.postgresql_session.rollback()
            await logger.exception("Failed to delete thread due to a database error", exc_info=True, thread_id=thread_id)
    ...
    async def retrieve_threads_by_user_id(self, user_id: str) -> List[Thread]:
        ...
        try:
            ...
            await logger.exception("Failed to retrieve threads for user_id due to a database error", exc_info=True, user_id=user_id)
            raise HTTPException(status_code=500, detail="Failed to retrieve threads for the specified user.")
