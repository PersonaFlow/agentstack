...
class UniqueConstraintError(Exception):
    """Exception raised for unique constraint violations in the assistant
    repository."""
...
class AssistantRepository(BaseRepository):
    ...
    async def create_assistant(self, data: dict) -> Assistant:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Failed to create assistant."
            ) from e

    @staticmethod
    def _get_retrieve_query() -> select:
        """A private method to construct the default query for assistant
        retrieval."""
        return select(
            Assistant.id,
            Assistant.user_id,
    ...
    async def retrieve_assistants(
        ...
        """Fetches all assistants. May include filters for narrowing results."""
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to retrieve assistants."
            )
    ...
    async def update_assistant(self, assistant_id: uuid.UUID, data: dict) -> Assistant:
        ...
        try:
            ...
            assistant = await self.update(
                model=Assistant, values=filtered_data, object_id=assistant_id
            )
            await self.postgresql_session.commit()
            return assistant
        except SQLAlchemyError as e:
    ...
    async def add_file_to_assistant(
        self, assistant_id: uuid.UUID, file_id: str
    ) -> Assistant:
        try:
            ...
            if assistant:
                ...
                updated_assistant = await self.update_assistant(
                    assistant_id, updated_data
                )
            ...
            raise HTTPException(
                status_code=500, detail="Failed to add file to assistant."
            )

    async def remove_file_reference_from_assistant(
        self, assistant_id: uuid.UUID, file_id: str
    ) -> Assistant:
        try:
            assistant = await self.retrieve_assistant(assistant_id)
            ...
            raise HTTPException(
                status_code=500, detail="Failed to remove file from assistant."
            )

    async def remove_all_file_references(
        self, file_id: uuid.UUID
    ) -> list[dict[str, Any]]:
        """Removes the file_id from all assistants that reference it.

        Returns a list of the id and name of the assistants that were
        updated.
        """
        updated_assistants = []
        try:
            filters = {"file_ids": {"contains": [str(file_id)]}}
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to remove file references from assistants.",
            )
