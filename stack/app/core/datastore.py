...
async def get_postgresql_session() -> AsyncIterator[AsyncSession]:
    ...
    This is used as a FastAPI dependency to get a database session for
    each request.
    """
    if _async_session_maker is None:
        raise RuntimeError(
            "Database not initialized. Ensure application startup has completed."
        )
...
async def initialize_checkpointer() -> None:
    ...
    This creates a connection pool for the checkpointer that will be
    used throughout the application lifecycle. The pool will be closed
...
def get_checkpointer() -> AsyncPostgresSaver:
    ...
    if _checkpointer is None:
        ...
        raise RuntimeError(
            "Checkpointer not initialized. Ensure application startup has completed."
        )
