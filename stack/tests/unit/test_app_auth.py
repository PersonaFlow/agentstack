...
async def test__list_auth_strategies__responds_correctly():
    # Arrange
    mock_strategies = {
        "Basic": MagicMock(get_client_id=lambda: None, get_authorization_endpoint=lambda: None, get_pkce_enabled=lambda: False),
        "Google": MagicMock(get_client_id=lambda: "google_client_id", get_authorization_endpoint=lambda: "https://accounts.google.com/o/oauth2/v2/auth", get_pkce_enabled=lambda: True),
        "OIDC": MagicMock(get_client_id=lambda: "oidc_client_id", get_authorization_endpoint=lambda: "https://example.com/auth", get_pkce_enabled=lambda: True)
    }
    ...
    assert google_strategy["authorization_endpoint"] == "https://accounts.google.com/o/oauth2/v2/auth"
