...
down_revision = 'e1338e81c83d'  # The ID of the previous revision. Specifies ordering of migrations.
...
def upgrade() -> None:
    # Add new columns
    op.add_column('checkpoints', sa.Column('thread_ts', sa.DateTime(timezone=True), nullable=True), schema='personaflow')
    op.add_column('checkpoints', sa.Column('parent_ts', sa.DateTime(timezone=True), nullable=True), schema='personaflow')

    # Update existing rows to set thread_ts
    op.execute("UPDATE personaflow.checkpoints SET thread_ts = CURRENT_TIMESTAMP AT TIME ZONE 'UTC' WHERE thread_ts IS NULL")

    # Drop the existing index on thread_id
    op.drop_index('ix_personaflow_checkpoints_thread_id', table_name='checkpoints', schema='personaflow')

    # Drop the existing primary key
    op.drop_constraint('checkpoints_pkey', 'checkpoints', schema='personaflow', type_='primary')

    # Add new primary key
    op.create_primary_key('checkpoints_pkey', 'checkpoints', ['thread_id', 'thread_ts'], schema='personaflow')
...
def downgrade() -> None:
    ...
    op.create_primary_key('checkpoints_pkey', 'checkpoints', ['id'], schema='personaflow')

    # Recreate the index on thread_id
    op.create_index('ix_personaflow_checkpoints_thread_id', 'checkpoints', ['thread_id'], unique=False, schema='personaflow')
