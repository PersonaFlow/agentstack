import { Query, QueryObserverResult, useMutation, useQuery, useQueryClient } from "@tanstack/react-query"
...
export const useGenerateTitle = (payload: t.TGenerateTitle) => {
    return useMutation({
        mutationFn: async(): Promise<t.TThread> => await dataService.generateTitle(payload),
    })
}
...
export const useUserThreads = (userId: string, grouped?:  boolean, timezoneOffset?: number) => {
...
export const useDeleteThread = (threadId: string) => {
    const queryClient = useQueryClient()
    return useMutation<void, Error>({
        mutationFn: async(): Promise<void> => await dataService.deleteThread(threadId),
        onSuccess: () => {
            queryClient.invalidateQueries({queryKey: [QueryKeys.threads]})
            queryClient.invalidateQueries({queryKey: [threadId]})
        }
...
export const useMessagesByThread = (threadId: string) => {
    return useQuery<t.TMessage[], Error>({
        ...
        queryFn: async(): Promise<t.TMessage[]> => await dataService.getMessagesByThread(threadId)
    })
}
...
export const useMessagesByCheckpoint = (threadId: string) => {
    return useQuery<t.TMessage[], Error>({
        ...
        queryFn: async(): Promise<t.TMessage[]> => await dataService.getMessagesByCheckpoint(threadId)
    })
}
...
export const useCreateAssistant = (payload: t.TAssistantRequest) => {
    ...
    return useMutation<t.TAssistant, Error>({
        mutationFn: async(): Promise<t.TAssistant> => await dataService.createAssistant(payload),
...
export const useUpdateAssistant = (assistantId: string, payload: t.TAssistantRequest) => {
    const queryClient = useQueryClient()
    return useMutation<t.TAssistant, Error>({
        mutationFn: async(): Promise<t.TAssistant> => await dataService.updateAssistant(assistantId, payload),
        onSuccess: () => {
...
export const useCreateAssistantFile = (assistantId: string, payload: t.TCreateAssistantFileRequest) => {
    ...
    return useMutation<t.TAssistantFile, Error>({
        mutationFn: async(): Promise<t.TAssistantFile> => await dataService.createAssistantFile(assistantId, payload),
        onSuccess: () => {
            queryClient.invalidateQueries({queryKey: [assistantId]})
        }
    })
...
export const useAssistantFiles = (assistantId: string, limit?: number, order?: string, before?: string, after?: string ) => {
    return useQuery<t.TFile[], Error>({
        queryKey: [QueryKeys.assistantFiles, assistantId],
        queryFn: async(): Promise<t.TFile[]> => dataService.getAssistantFiles(assistantId, limit, order, before, after)
    })
}
...
export const useDeleteAssistantFile = (assistantId: string, fileId: string) => {
    ...
    return useMutation<t.TAssistant>({
        mutationFn: async(): Promise<t.TAssistant> => dataService.deleteAssistantFile(assistantId, fileId),
        onSuccess: () => {
...
export const useIngestFileData = (payload: t.TIngestFileDataRequest) => {
    return useMutation<{}, Error>({
        mutationFn: async(): Promise<{}> => await dataService.ingestFileData(payload)
    })
}
...
export const useQueryLCRetriever = (payload: t.TQuery) => {
    return useMutation<string, Error>({
        mutationFn: async(): Promise<string> => await dataService.queryLCRetriever(payload)
    })
}
...
export const useUploadFile = (payload: FormData) => {
    ...
    return useMutation<t.TFile, Error>({
        mutationFn: async (): Promise<t.TFile> => await dataService.uploadFile(payload),
        onSuccess: () => {
...
export const useFiles = (userId: string, purpose?: t.TPurpose) => {
    return useQuery<t.TFile[], Error>({
        ...
        queryFn: async (): Promise<t.TFile[]> => await dataService.getFiles(userId, purpose)
    })
}
...
export const useDeleteFile = (fileId: string) => {
    ...
    return useMutation<t.TDeleteFileResponse, Error>({
        mutationFn: async (): Promise<t.TDeleteFileResponse> => await dataService.deleteFile(fileId),
        onSuccess: () => {
            queryClient.invalidateQueries({queryKey: [fileId]})
            queryClient.invalidateQueries({queryKey: [QueryKeys.files]})
        }
