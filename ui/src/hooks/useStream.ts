...
export const useStream = () => {
  ...
  const startStream = useCallback(async ({ input, thread_id, assistant_id, user_id }: TStartStreamProps) => {
    const controller = new AbortController()
    setController(controller)
    setCurrentState({ status: TFileStreamStatus.inflight, messages: [] })

    await fetchEventSource(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/v1/runs/stream`, {
