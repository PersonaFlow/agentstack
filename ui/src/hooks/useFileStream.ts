...
import { TStreamState } from "@/data-provider/types";
import { fetchEventSource } from "@microsoft/fetch-event-source";
import { useCallback, useEffect, useState } from "react";

type TStartFileStreamProps = {
    taskId: string;
}
...
export const useFileStream = () => {
  ...
  const startFileStream = useCallback(
    async ({ taskId }: TStartFileStreamProps) => {
      ...
      await fetchEventSource(
        `${process.env.NEXT_PUBLIC_BASE_API_URL}/api/v1/rag/ingest/${taskId}/progress`,
        {
