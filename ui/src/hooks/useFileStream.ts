...
import { TFileIngest, TFileStreamStatus, TFileStreamProgressState } from '@/data-provider/types'
import { fetchEventSource } from '@microsoft/fetch-event-source'
import { useQueryClient } from '@tanstack/react-query'
import { useCallback, useState } from 'react'

export const useFileStream = () => {
  const [currentState, setCurrentState] = useState<TFileStreamProgressState | null>(null)
  ...
  const startProgressStream = useCallback(async ({ task_id }: TFileIngest) => {
    ...
    await fetchEventSource(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/v1/rag/ingest/${task_id}/progress`, {
      ...
      onclose() {
        setCurrentState((currentState) => ({
          ...
          progress:
            currentState?.status === TFileStreamStatus.error
